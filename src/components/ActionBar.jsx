    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-100">
      <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={onRandom}
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-gray-400 hover:text-black hover:bg-gray-50 transition-all"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <button
          onClick={handleCopy}
          disabled={count < 1}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold bg-black text-white hover:bg-gray-800 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
        >
          {copied ? (
            <>
            ...
            <>
              ...
              Copy Prompt
              {count > 0 && (
                <span className="bg-white text-black text-xs font-bold px-1.5 py-0.5 rounded-md leading-none">
                  {count}
                </span>
              )}
            </>
